...
const (
	// Keys in NLG-IBFT Configuration
	KeyType          = "type"
	KeyTypes         = "types"
...
var (
	ErrUndefinedIBFTConfig = errors.New("NLG-IBFT config is not defined")
	errInvalidBlockTime    = errors.New("invalid block time provided")
)

// NLG-IBFT Fork represents setting in params.engine.ibft of genesis.json
...
// GetIBFTForks returns NLG-IBFT fork configurations from chain config
func GetIBFTForks(ibftConfig map[string]interface{}) (IBFTForks, error) {
	// no fork, only specifying NLG-IBFT type in chain config
