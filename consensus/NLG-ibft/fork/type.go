...
// Define the type of the NLG-IBFT consensus
type IBFTType string

const (
	// PoA defines the Proof of Authority NLG-IBFT type,
	// where the validator set is changed through voting / pre-set in genesis
	PoA IBFTType = "PoA"

	// PoS defines the Proof of Stake NLG-IBFT type,
...
// ibftTypesToSourceType defines validator set type used under each NLG-IBFT Type
// Right now each NLG-IBFT Type is correspond one-to-one with ValidatorStore
...
func ParseIBFTType(ibftType string) (IBFTType, error) {
	...
	if !ok {
		return castType, fmt.Errorf("invalid NLG-IBFT type %s", ibftType)
	}
