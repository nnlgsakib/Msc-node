...
import (
	...
	"github.com/Mind-chain/mind/consensus/NLG-ibft/proto"
	...
	"github.com/Mind-chain/mind/syncer"
...
var (
	ErrInvalidHookParam           = errors.New("invalid NLG-IBFT hook param passed in")
...
// backendIBFT represents the NLG-IBFT consensus mechanism object
type backendIBFT struct {
	...
	operator       *operator              // Reference to the gRPC service of NLG-IBFT
	transport      transport              // Reference to the transport protocol

	// Dynamic References
	forkManager       forkManagerInterface  // Manager to hold NLG-IBFT Forks
...
// Start starts the NLG-IBFT consensus
...
// Close closes the NLG-IBFT consensus mechanism, and does write back to disk
...
// SetHeaderHash updates hash calculation function for NLG-IBFT
...
func (i *backendIBFT) logFork(
	...
	if lastSigner != nil && signer != nil && lastSigner.Type() != signer.Type() {
		i.logger.Info("NLG-IBFT validation type switched", "old", lastSigner.Type(), "new", signer.Type())
	}
}
...
// verifyProposerSeal verifies ProposerSeal in NLG-IBFT Extra of header
